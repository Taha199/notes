...
/**
 * id -> soft-delete timestamp (ms). A durable proof that a set/folder was
...
async function fetchCloudTrashTombstones(uid: string): Promise<{ sets: TrashTombstones; folders: TrashTombstones }> {
  try {
...
interface NotesCtx {
  ...
  loaded: boolean;
  /**
   * Quiz lists are safe to paint after local IDB (+ optional same-session cache)
   * hydrate — does NOT wait for the full cloud round-trip (`loaded`).
   */
...
export function NotesProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    // That cache already survived a prior ById merge in this SPA lifetime — safe for
    // both structure and question bodies (no old→new FOUC within the tab).
    if (
      quizListsBootCache
    ...
    (async () => {
      try {
        ...
        rawSets = applyTrashTombstones(rawSets, quizSetTombstonesRef.current);
