...
export function ChatPage() {
  ...
  const updateChats = (updated: ChatConversation[]) => {
    setChats(updated);
    saveChats(updated);
  };
  ...
  const sendMessage = async () => {
    ...
    let isNew = false;
    if (!conv) {
      conv = { id: newId(), title: deriveTitleFromMessage(text), messages: [], createdAt: nowStr() };
      isNew = true;
    }

    const userMsg: ChatMessage = { role: 'user', text, timestamp: nowStr() };
    const updatedMessages = [...conv.messages, userMsg];
    const updatedConv: ChatConversation = {
      ...conv,
      title: conv.messages.length === 0 ? deriveTitleFromMessage(text) : conv.title,
    ...
    <div className="flex h-full overflow-hidden">
          ...
          {chats.length === 0 ? (
            ...
            chats.map((c) => (
              <div key={c.id} className="group relative mb-0.5">
                ...
                >
                  <span className="w-full truncate text-[12.5px] font-semibold leading-snug pr-5">{c.title}</span>
                  <span className="mt-0.5 text-[10.5px] opacity-60">{c.createdAt}</span>
                </button>
                <button
                  onClick={() => deleteChat(c.id)}
                  className="absolute right-2 top-1/2 -translate-y-1/2 hidden rounded-md p-1 text-[11px] text-app-text-secondary/40 hover:bg-red-50 hover:text-red-500 group-hover:flex dark:hover:bg-red-500/10"
                  title="Delete"
                >✕</button>
              </div>
            ))
      ...
      <div className="flex min-w-0 flex-1 flex-col bg-white dark:bg-gray-900">
        ...
        <div className="flex min-h-[52px] flex-shrink-0 items-center gap-3 border-b border-app-border px-4 dark:border-white/10">
          ...
          <span className="text-[14px] font-semibold text-app-text dark:text-gray-100">
            {activeChat ? activeChat.title : 'AI Chat'}
          </span>
          {activeChat && (
            <span className="ml-auto text-[11px] text-app-text-secondary/50 dark:text-gray-600">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-6">
          {!activeChat || activeChat.messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-[13px] text-app-text-secondary dark:text-gray-500">Start a conversation — ask anything.</p>
              <div className="mt-6 grid grid-cols-1 gap-2 sm:grid-cols-2">
                {[
                  'Summarize my notes for me',
                  'Help me study for an exam',
                  'Explain a concept simply',
                  'Write a plan for my week',
                ].map((s) => (
                  <button
                    key={s}
            ...
            <div className="mx-auto flex max-w-2xl flex-col gap-5">
              ...
              {error && (
                <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-[13px] text-red-600 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-400">
                  ⚠️ {error}
                </div>
              )}
              <div ref={bottomRef} />
        ...
        <div className="flex-shrink-0 border-t border-app-border bg-white px-4 py-3 dark:border-white/10 dark:bg-gray-900">
          <div className="mx-auto flex max-w-2xl items-end gap-2 rounded-2xl border border-app-border bg-app-bg px-4 py-2.5 shadow-sm transition-all focus-within:border-primary/50 focus-within:bg-white focus-within:ring-4 focus-within:ring-primary/10 dark:border-white/10 dark:bg-white/5 dark:focus-within:bg-gray-800">
            <textarea
              ref={textareaRef}
              value={input}
            ...
            <button
              ...
              className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-xl bg-primary text-white shadow-md shadow-primary/30 transition-all hover:-translate-y-0.5 hover:bg-primary-dark disabled:cursor-not-allowed disabled:opacity-40"
              title="Send"
            >
